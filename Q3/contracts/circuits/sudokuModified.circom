...
template sudoku() {
    ...
    component rp[9][9]; //= RangeProof(32,9,9);
    component rp2[9][9];// = RangeProof(32,9,9);
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            //rp1.in <== puzzle[i][j]; //assert(puzzle[i][j]>=0); //[assignment] change assert() to use your created RangeProof instead
            //assert(puzzle[i][j]<=9); //[assignment] change assert() to use your created RangeProof instead
            //rp2.in <== solution[i][j]; //assert(solution[i][j]>=0); //[assignment] change assert() to use your created RangeProof instead
